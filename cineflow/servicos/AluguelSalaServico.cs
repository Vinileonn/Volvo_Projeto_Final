...
namespace cineflow.servicos
{
    public class AluguelSalaServico
    {
        ...
        public List<AluguelSala> ListarPorCliente(Cliente cliente)
        {
            return alugueis.Where(a => a.Cliente != null && a.Cliente.Id == cliente.Id).ToList();
        }
        ...
        private bool ExisteConflitoAluguel(Sala sala, DateTime inicio, DateTime fim)
        {
            return alugueis.Any(a => a.Sala.Id == sala.Id &&
                inicio < a.Fim && fim > a.Inicio && a.Status != StatusAluguel.Cancelado);
        }
